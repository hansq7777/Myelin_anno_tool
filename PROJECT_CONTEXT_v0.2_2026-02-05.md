...
### 当前阶段目标（新增：预测质控与训练集扩充）

本阶段重点不是再开新模型分支，而是把当前“表现优秀模型”的推理结果转化为下一轮训练可直接利用的数据资产。目标如下：

- 从已推理完成的“原图 + 预测”配对中，筛出完成度高、需要人工干预少、质量稳定的样本。
- 对候选样本进行快速微调校正（最小必要人工修改），形成高可信标注。
- 按可用性分级（例如 A/B/C）管理样本，优先将 A/B 级纳入下一轮微调训练集。
- 建立可追溯的数据策展记录（样本ID、质控等级、人工修改量、是否入库训练）。

建议的本阶段验收标准：

- 样本级全量“原图-预测”一一匹配检查通过（缺失为 0，或有明确排除名单）。
- 每个样本完成质控标签（如 `accept_direct` / `accept_after_edit` / `reject`）。
- 形成至少一版可复用训练清单（包含 `train/val` 划分和样本质量等级）。
...
- 历史上曾出现个别文件缺失 OME‑XML 的情况（例如早期 `2504_42_L_M1_S28.ome.tif`），当前批次已做补齐与标准化；若后续新增批次再出现缺失，仍按同 ROI 元信息做 **dz 参考** 并记录来源。
...
### 2.8 Review 维度对齐策略（2026-02-07 更新）

当前 GUI 的 review 打开流程统一为 **mask-grid 对齐**：

- 加载配对数据时，先读取预测/标注栈维度（目标维度）。
- 原始 zstack 在内存中按目标维度下采样/重采样后再 overlay 显示。
- 人工修改后的掩膜按预测维度保存（不回写为原始 raw 维度）。

这样可直接对齐下游使用统一 downsampled `dz` 的训练流程，避免“review 阶段维度”与“训练阶段维度”不一致。

元信息现状（基于当前 665 对配对文件扫描）：

- raw OME-TIFF：`665/665` 均含 OME metadata，且均可读出 `dx/dy/dz`，可计算物理长度（`Lx=dx*SizeX`，`Ly=dy*SizeY`，`Lz=dz*SizeZ`）。
- inference OME-TIFF：`665/665` 文件存在 OME metadata，但未写入 `PhysicalSizeX/Y/Z`，因此不能仅凭 inference 文件独立恢复物理长度，需关联 raw 元数据。
